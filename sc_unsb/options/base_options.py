import argparse
import os
from sc_unsb.utils import util
import torch
import models
import data
...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--normG', type=str, default='instance', choices=['instance', 'batch', 'none', 'dn'], help='instance normalization or batch normalization for G')
        ...
        # V2 patch-based inference parameters
        parser.add_argument('--experiment', type=str, default='v1', help='experiment type: v1 (resize) or v2 (patch)')
        # Note: --patch_size and --overlap are defined in test_options.py for V1/V2 inference
        # DN patch-based inference parameters
        parser.add_argument('--dn_prefetch', action='store_true', help='use DN prefetch (single-pass) inference')
        parser.add_argument('--dn_patch_size', type=int, default=512, help='patch size for DN inference')
        parser.add_argument('--dn_stride', type=int, default=512, help='stride for DN inference')
        parser.add_argument('--dn_padding', type=int, default=1, help='padding for DN 3x3 interpolation')
