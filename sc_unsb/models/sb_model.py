...
class SBModel(BaseModel):
    ...
    def enable_dn_plugin(self):
        """启用DN插件（用于颜色统计收集，不用于推理）"""
        from .dn import replace_instance_norm_with_dn
        replace_instance_norm_with_dn(self.netG)
        count = sum(1 for m in self.netG.modules() if hasattr(m, 'collection_mode'))
        return count

    def init_color_stats_collection(self, n_h, n_w, num_timesteps=1):
        """初始化颜色统计量收集（用于后处理颜色插值）"""
        self.color_stats = {}
        self.n_h = n_h
        self.n_w = n_w
        self.num_timesteps = num_timesteps

    def collect_patch_color_stats(self, patch_tensor, y_anchor, x_anchor, timestep):
        """收集单个patch的颜色统计量"""
        key = (y_anchor, x_anchor, timestep)
        mean = patch_tensor.mean(dim=(2, 3), keepdim=True)
        std = patch_tensor.std(dim=(2, 3), keepdim=True) + 1e-5
        self.color_stats[key] = {'mean': mean.detach(), 'std': std.detach()}

    def apply_color_interpolation(self, patch_tensor, y_anchor, x_anchor, timestep, padding=1):
        """对生成的patch应用颜色插值调整（RGB 3通道）"""
        import torch
        from .interpolation import Interpolation3D

        C = patch_tensor.shape[1]  # 通道数
        H, W = patch_tensor.shape[2], patch_tensor.shape[3]

        # 收集3x3邻域的统计量（每个通道独立）
        mean_grid = torch.zeros(C, 3, 3, device=patch_tensor.device)
        std_grid = torch.zeros(C, 3, 3, device=patch_tensor.device)

        for dy in range(-padding, padding+1):
            for dx in range(-padding, padding+1):
                ny, nx = y_anchor + dy, x_anchor + dx
                key = (ny, nx, timestep)
                if key in self.color_stats:
                    stats = self.color_stats[key]
                    mean_grid[:, dy+padding, dx+padding] = stats['mean'].squeeze()
                    std_grid[:, dy+padding, dx+padding] = stats['std'].squeeze()

        # 插值到像素级（每个通道独立插值）
        interp = Interpolation3D(channel=C)
        interp.init(H, patch_tensor.device)

        mean_field = interp.interpolation_mean_table(mean_grid)  # [C, H, W]
        std_field = interp.interpolation_std_table(std_grid)      # [C, H, W]

        # 颜色调整：先标准化到当前patch统计，再映射到插值统计
        current_mean = patch_tensor.mean(dim=(2, 3), keepdim=True)
        current_std = patch_tensor.std(dim=(2, 3), keepdim=True) + 1e-5

        normalized = (patch_tensor - current_mean) / current_std
        adjusted = normalized * std_field.unsqueeze(0) + mean_field.unsqueeze(0)

        return adjusted

    def forward_with_anchor(self, x, time_idx, z, y_anchor=0, x_anchor=0, padding=1, dn_time_idx=0):
        """标准前向传播（不使用DN，保证细胞结构完整）"""
        return self.netG(x, time_idx, z)
