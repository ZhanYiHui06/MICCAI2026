...
def get_norm_layer(norm_type='instance', opt=None):
    ...
    if norm_type == 'batch':
        ...
        use_prefetch = bool(getattr(opt, 'dn_prefetch', False)) if opt is not None else False
        norm_cls = PrefetchDenseInstanceNorm if use_prefetch else DenseInstanceNorm
        norm_layer = functools.partial(norm_cls, affine=False)
    elif norm_type == 'none':
        def norm_layer(x):
            return Identity()
    else:
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    """
    net = None
    norm_layer = get_norm_layer(norm_type=norm, opt=opt)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func in (nn.InstanceNorm2d, DenseInstanceNorm, PrefetchDenseInstanceNorm)
        else:
            use_bias = norm_layer in (nn.InstanceNorm2d, DenseInstanceNorm, PrefetchDenseInstanceNorm)
    ...
    def _forward_dn(self, x, modules, y_anchor, x_anchor, padding=1, **kwargs):
        for layer in modules:
            if isinstance(layer, (DenseInstanceNorm, PrefetchDenseInstanceNorm)):
                x = layer(x, y_anchor=y_anchor, x_anchor=x_anchor, padding=padding, **kwargs)
            elif hasattr(layer, "forward_with_anchor"):
                x = layer.forward_with_anchor(x, y_anchor, x_anchor, padding=padding, **kwargs)
            else:
                x = layer(x)
        return x

    def forward_with_anchor(self, x, y_anchor, x_anchor, padding=1, **kwargs):
        return self._forward_dn(x, self.model, y_anchor, x_anchor, padding=padding, **kwargs)
...
class ResnetBlock(nn.Module):
    ...
    def forward_with_anchor(self, x, y_anchor, x_anchor, padding=1, **kwargs):
        ...
        for layer in self.conv_block:
            if isinstance(layer, (DenseInstanceNorm, PrefetchDenseInstanceNorm)):
                out = layer(out, y_anchor=y_anchor, x_anchor=x_anchor, padding=padding, **kwargs)
            else:
                out = layer(out)
        out = x + out
        return out
